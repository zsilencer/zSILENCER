...
void Player::Tick(World & world){
	...
	if(input.keychat && !chatinterfaceid && !buyinterfaceid && this == world.GetPeerPlayer(world.localpeerid)){
	...
	if(input.keydisguise && !oldinput.keydisguise){
		if(disguised >= 100){
			...
			if(state != JETPACK && state != DEPLOYING){
				disguised = 112;
				Audio::GetInstance().EmitSound(id, world.resources.soundbank["disguise.wav"], 64);
				if(OnGround()){
					state = RUNNING;
					state_i = 25;
				}
	...
	if(hacksoundchannel != -1){
		if(state != HACKING || state_i >= 17){
			if(!world.replaying){
				Audio::GetInstance().EmitSound(id, world.resources.soundbank["jackout.wav"], 15);
	...
	if(jetpacksoundchannel != -1){
		if(state != JETPACK && state != JETPACKSHOOT){
			if(!world.replaying){
				...
				Audio::GetInstance().EmitSound(id, world.resources.soundbank["jetpak2a.wav"], 64);
	...
	if(oldhassecret != hassecret && !world.replaying){
		...
		if(hassecret){
			Audio::GetInstance().Play(world.resources.soundbank["alinvest.wav"]);
			if(peer && !world.intutorialmode){
				User * user = world.lobby.GetUserInfo(peer->accountid);
				char text[64];
				sprintf(text, "Secret picked up by\n%s", user->name);
				world.ShowMessage(text);
	...
	switch(state){
		...
		case RUNNING:{
			...
			if(IsDisguised() && state_i >= 25 && oldx == x){
				mirrored = !mirrored;
			}
			if(state_i >= 0 && state_i <= 5){
				if(IsDisguised()){
					...
					res_index = state_i;
					if(res_index == 3){
						Audio::GetInstance().EmitSound(id, world.resources.soundbank["futstonl.wav"], 24);
					}
				}
			}
			if(state_i > 5 && state_i <= 20){
				...
				if(res_index == 11){
					Audio::GetInstance().EmitSound(id, world.resources.soundbank["futstonr.wav"], 24);
				}
			}
		...
		case WALKIN:{
			...
			res_index = state_i;
			if(state_i == 1){
				Audio::GetInstance().EmitSound(id, world.resources.soundbank["portpas2.wav"], 32);
			}
			if(state_i >= 15){
				collidable = true;
				if(basedoor){
		...
		case ROLLING:{
			...
			res_index = state_i;
			if(res_index == 0){
				Audio::GetInstance().EmitSound(id, world.resources.soundbank["roll2.wav"], 32);
			}
			if(state_i >= 8){
				xv = 0;
				res_bank = 18;
		...
		case JUMPING:{
			...
			Audio::GetInstance().EmitSound(id, world.resources.soundbank["juunewne.wav"], 96);
			yv = v;
			xv2 = xv;
			yv2 = yv;
		...
		case HACKING:{
			...
			if(hackable && terminal){
				if(state_i == 14){
					Audio::GetInstance().EmitSound(id, world.resources.soundbank["jackin.wav"], 15);
					if(hacksoundchannel == -1){
			...
			if(state_i > 48 && !state_warp && !world.winningteamid){
				...
				if(team && team->agency == Team::LAZARUS && canresurrect && !world.winningteamid){
					...
					if(team && team->basedoorid){
						...
						health = maxhealth;
						shield = maxshield;
						Audio::GetInstance().EmitSound(id, world.resources.soundbank["transrev.wav"], 64);
						state_i = -1;
						break;
					}else{
						Sint16 x;
		...
		case RESURRECTING:{
			...
			res_index = state_i;
			if(state_i == 2){
				Audio::GetInstance().EmitSound(id, world.resources.soundbank["repair.wav"], 128);
			}
			if(state_i >= 27){
				health = maxhealth;
				collidable = true;
...
void Player::HandleHit(World & world, Uint8 x, Uint8 y, Object & projectile){
	...
	if(world.tickcount - hitsoundplaytick > 10){
		if(rand() % 2 == 0){
			Audio::GetInstance().EmitSound(id, world.resources.soundbank["s_hita01.wav"]);
		}else{
			Audio::GetInstance().EmitSound(id, world.resources.soundbank["s_hitb01.wav"]);
		}
	...
	if(hit_type == 1){
		Audio::GetInstance().EmitSound(id, world.resources.soundbank["shlddn1.wav"]);
	}
	if(health == 0 && state != DYING && state != DEAD){
			...
			if(owner){
				switch(owner->type){
					case ObjectTypes::WALLDEFENSE:{
						killedby = "a Wall Defense Turret";
					}break;
		...
		state = DYING;
		state_i = 0;
		Audio::GetInstance().EmitSound(id, world.resources.soundbank["grunt2a.wav"]);
	}
}
...
bool Player::BuyItem(World & world, Uint8 id){
	...
	if(buyableitem && credits >= buyableitem->price){
		...
		if(bought){
			world.audio.EmitSound(Player::id, world.resources.soundbank["reload2.wav"], 96);
			world.BuyItem(id);
		...
		switch(currentweapon){
			...
			case 3:{
				if(flamerammo == 0){
					...
					if(flamersoundchannel == -1){
						flamersoundchannel = world.audio.EmitSound(id, world.resources.soundbank["flamebg2.wav"], 128, true);
					}
					flamerammo--;
				}
			}break;
...
bool Player::ProcessJetpackState(World & world){
	...
	if(state_i == 0){
		y--;
		if(world.map.TestAABB(x, y - height, x, y, Platform::RECTANGLE | Platform::STAIRSUP | Platform::STAIRSDOWN)){
			y++;
			state = STANDING;
			state_i = -1;
			return true;
	...
	if(platform){
		if(CheckForGround(world, *platform)){
			...
			if(abs(xv) >= 2 || abs(yv) >= 2){
				world.audio.EmitSound(id, world.resources.soundbank["land1.wav"], 96);
			}
		}
	}
	if(yv > 0){
...
bool Player::ProcessFallingState(World & world){
	...
	if(platform){
		if(CheckForGround(world, *platform)){
			...
			yv = (yn * abs(yv)) / 2;
			if(abs(xv) >= 2 || abs(yv) >= 2){
				world.audio.EmitSound(id, world.resources.soundbank["fall2b.wav"], 96);
			}
		}
	}else{
		if(CheckForLadder(world)){
...
bool Player::ProcessLadderState(World &world){
	...
	res_index = state_i;
	if(res_index == 4){
		world.audio.EmitSound(id, world.resources.soundbank["ladder1.wav"], 24);
	}
	if(res_index == 15){
		world.audio.EmitSound(id, world.resources.soundbank["ladder2.wav"], 24);
	}
	return false;
}
...
void Player::Warp(World & world, Sint16 x, Sint16 y){
	if(!state_warp){
		...
		warpy = y;
		state_warp = 1;
		world.audio.EmitSound(id, world.resources.soundbank["transrev.wav"], 96);
	}
}
...
void Player::PickUpItem(World & world, PickUp & pickup){
	...
	switch(pickup.type){
		...
		case PickUp::FILES:{
			...
			effecthacking = true;
			effecthackingcontinue = 5;
			world.audio.EmitSound(id, world.resources.soundbank["power11.wav"], 64);
		}break;
		case PickUp::LASERAMMO:{
			...
			world.audio.EmitSound(id, world.resources.soundbank["reload2.wav"], 96);
		}break;
